...
haunted_data <- haunted_data %>%
  ...
  mutate(Mean_Symptom = (post_SympSum+pre_SympSum)/2) %>%
...
corr_data <- haunted_data %>%
  select(matches("SympSum"),matches("RateScared"),matches("Diff_Fear"),matches("Diff_Symptom"),
         matches("Precision"), matches("Threshold"), matches("MedianBPM"), matches("Mean_Fear"), matches("Mean_Symptom"), matches("RapidHB"), matches("Diff_HB"))


#corr_data <- corr_data[, c(7, 8, 1,3,12, 2,4,13, 10,11, 14, 5,6,9)]
corr_data <- corr_data[, c(7, 8, 1,3,12, 2,4,13,5,6,14,9)]
colnames(corr_data) <- c("Precision", "Threshold", "preAnxiety", "preFear", "preHB", "postAnxiety", "postFear ", "postHB", "diffAnx","diffFear", "diffHB", "medianBPM") 

pval <- psych::corr.test(corr_data, method = "spearman", adjust = "fdr")$p


cp1 <- ggcorrplot(cor(corr_data), p.mat = pval, hc.order=FALSE, type='full', insig='blank', method = "square", sig.level = 0.05, 
           outline.col = "black",
...
ggsave(here("figs", "fig_3corrplot_fdr_05.png"), height = 8, width = 8, dpi = 150, units = "in")
