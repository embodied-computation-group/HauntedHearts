...
plot_data <- haunted_data %>%
  select(matches("RateScared"), matches("SympSum")) %>%
...
p3 <- ggplot(rate_data, aes(x = Measure, y = Rating, fill = Time)) +
  geom_boxjitter(notch = TRUE, width = .5, jitter.shape = 1, outlier.shape = NA) +
  scale_fill_manual(values = c("#E7B800", "#FC4E07")) +
  theme_cowplot()+
...
ggsave(here("figs", "surveyResults2.png"), height = 6, width = 12, dpi = 150, units = "in")
```
